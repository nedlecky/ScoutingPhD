...
void main(int argc,char** argv)
{
	...
	printf("%d basic params, %d diffs, %d sums (%d total)\n",
		nBasicParams,nDifferenceParams,nSumParams,nParams);
	int imageDx,imageDy;
	char image_file[256];
	...
	int best=40;
	for(int scout=0; scout<100; scout++)
	{
		if(ScoutTrial(100,best-1,nParams,0,nParams-1,nExpected,blobstat))
		{
			best--;
			scout--;
		}
...
void BlobStat::AddBlob(Blob* blob,Image* im)
{
	...
	for(int row=0; row<4; row++)
		...
		//mvt_constant(roi,128);
			val[nextStat]=cnt/structSize;
			AddStat(nextStat++,val[nextStat]);
		}

	// Difference Computations (120 of these; 16 choose 2)
	for(int p1=0; p1<15; p1++)
		for(int p2=p1+1; p2<16; p2++)
			if(p1!=p2)
			{
				val[nextStat]=2+val[p1]-val[p2];
				AddStat(nextStat++,val[nextStat]);
			}
...
/*
	int nPairs=nextStat;
	for(p1=0; p1<nPairs; p1++)
		AddStat(nextStat++,val[p1]+val[p1+1]);
*/
	im_delete(roi);
	delete val;
	N++;
