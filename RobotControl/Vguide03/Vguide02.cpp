...
double amp2;
double fact;
LEmatrixObj& ComputeXd(int targetX,int targetY)
{
	static LEmatrixObj Xd("Xd",6,1);
...
LEmatrixObj& MakeNNx(LEmatrixObj& e,LEmatrixObj& edot,LEmatrixObj& qd)
{
	static LEmatrixObj NNx("NNx");
...
int Guidance(void)
{
	...
	int L=100;
	int m=tau.mRows();
	LEmatrixObj V("V",n+1,L);
	LEmatrixObj W("W",L+1,m);
	printf("n=%d L=%d m=%d\n",n,L,m);
	V=0;
	W=0;
	...
	while (fContinue)
	{
		...
		if(fTrack && nFails==0)
		{
			...
			double dt=t-tLast;
			Xd=ComputeXd(targetX[curTarget],targetY[curTarget]); // pos1 pos2 vel1 vel2 acc1 acc2
			...
			if(fTrainNet)
			{
				...
				for(int i=1; i<del1.mRows(); i++) del1[i-1][0]=del1[i][0];
				del1.SetSize(del1.mRows()-1,del1.nCols());

				W = W + eta2*z*lemTranspose(del2);
				V = V + eta1*NNx*lemTranspose(del1);
