...
int manhTable[10][9][2];

// Load manhTable with goal state info.
// [t][0] is row tile t should be in
// [t][1] is col tile t should be in
void SetManhattan(int* goal,int t=0)
{
	int i=0;
	for(int r=0; r<3; r++)
		for(int c=0; c<3; c++)
		{
			manhTable[t][goal[i]][0]=r;
			manhTable[t][goal[i]][1]=c;
			i++;
		}
}
void SetManhattan(int p,int t=0)
{
	int tiles[9];
	PExpand(tiles,p);
	SetManhattan(tiles,t);
}

// Compute manhattan distance; must call SetManhattan with goal first
int ManhDistance(int* tile,int t=0)
{
	int dist=0;
	for(int i=0; i<9; i++)
		if(tile[i])
		{
			int r=i/3;
			int c=i%3;
			dist = dist + abs(r-manhTable[t][tile[i]][0]) + abs(c-manhTable[t][tile[i]][1]);
		}
	return dist;
}
int ManhDistance(int p,int t=0)
{
	int tile[9];
	PExpand(tile,p);
	return ManhDistance(tile,t);
}
...
void PuzzleList::TrainingData(char* fileprefix)
{
	...
	for(int i=0; i<size; i++)
	{
		...
		for(int t=0; t<10; t++)
			Xbar[t][i]=ManhDistance(tiles,t);
		Y[0][i]=depths[i];
	}

	char filename[128];
	sprintf(filename,"%s_Xbar.csv",fileprefix);
	Xbar.Save(filename);
...
int PuzzleList::RandomPuzzle(void)
{
	return puzzles[leRandInt(0,size-1)];
}
...
void TrainNN(void)
{
	...
	while(epoch<maxEpochs)
	{
		...
		for(int c=0/*leRandInt(0,1000)*/; c<X.Ncols(); c+=1000)
		{
			int doCol=c;
			LEmatrix x("x");
			x=leCol(X,doCol);
...
int main(int argc, char* argv[])
{
	...
	if(0)
	{
		for(int d=3; d<=30; d+=3)
		{
			...
			int hard=initStates.RandomPuzzle();
			
			printf("Adding test puzzle %d (%s)\n",i,PString(hard));
			SetManhattan(hard,i);
		}
