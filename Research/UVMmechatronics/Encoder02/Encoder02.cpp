// Encoder02.cpp : Defines the entry point for the console application.
//
...
int main(int argc, char* argv[])
{
	...
	for(Ts=0.0005; Ts<=0.1; Ts+=0.0005)
	{
		...
		xe=0;
		a=v=x=0;
		printf("Ts=%8.4lf omegan=%8.3lf damping=%8.3lf   K1=%11.3lf K2=%9.3lf ",Ts,omegan,damping,K1,K2);
		...
		while(t<tf)
		{
			...
			// Lee and Song, corrected MSE 0.002 0.006 0.006
			ae = K1*(x-xe) - K2*ve;
			vep = ve + ae*Ts;
			xep = xe + ve*Ts + 0.5*ae*Ts*Ts;

			// Idea #1 MSE 0.002 0.002 0.001
			//ae = K1*(x-xe) - K2*ve;
			//vep = ve + ae*Ts;
			//xep = xe + ve*Ts;

			// Idea #2 MSE 0.002 0.010 0.015
			//ae = K1*(x-xe) - K2*ve;
			//vep = ve + ae*Ts;
			//xep = xe + vep*Ts;
			...
			fprintf(fp,"%.2lf,%.3lf,%.3lf,%.3lf,%.3lf,%.3lf,%.3lf\n",
				t,xErr,vErr,aErr,
				x,v,a);
