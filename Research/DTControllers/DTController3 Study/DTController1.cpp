...
int main(int argc, char* argv[])
{
	...
	for(int i=0; i<3; i++)
	{
		...
		while(t<tf)
		{
			...
			double x2diff=v0 + a0/2*updateDt;
			double x3diff=a0 +(a0-a1)*updateDt;
			...
			// Save vel vs estimates
			if(fFirstTime)
				diff.SetSize(1,6);
			else
				diff.SetSize(diff.Nrows()+1,6);
			r=diff.Nrows()-1;
			diff[r][0]=t;
			diff[r][1]=x1;
			diff[r][2]=x2;
		...
		int newNcols=result.Ncols()+4;
		result.SetSize(result.Nrows(),newNcols);
		for(int j=0; j<result.Nrows(); j++)
		{
			LEmatrix traj;
			traj=ComputeXd(result[j][0]);
			result[j][newNcols-4]=traj[1][0];
			result[j][newNcols-3]=result[j][1]-traj[1][0];
			result[j][newNcols-2]=traj[2][0];
			result[j][newNcols-1]=result[j][2]-traj[2][0];
		}
		...
		sprintf(filename,"diff_%s.csv",result.Name());
		diff.Save(filename);
	}
